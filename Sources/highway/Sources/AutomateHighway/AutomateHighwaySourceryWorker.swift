...
public class AutomateHighwaySourceryWorker: AutomateHighwaySourceryWorkerProtocol, AutoGenerateProtocol
{
    ...
    ) throws
    {
        ...
        let swiftFormatGeneratedCodeWorker = try SwiftFormatWorker(forAutogeneratedCode: ProjectFolderWorker.self, queue: AutomateHighwaySourceryWorker.queue)

        self.workers = try sourcerySequence.map
        {
            try SourceryWorker(sourcery: $0, swiftFormatForAutoGeneratedCodeWorker: swiftFormatGeneratedCodeWorker, queue: AutomateHighwaySourceryWorker.queue)
        }
    }
