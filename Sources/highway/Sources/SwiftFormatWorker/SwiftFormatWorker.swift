...
public class SwiftFormatWorker: SwiftFormatWorkerProtocol, AutoGenerateProtocol
{
    ...
    ) throws
    {
        ...
        signPost.verbose("👨🏻‍🏫 Running in init in folder\n\(currentFolder.name)\n")
        do
        {
        ...
        {
            signPost.verbose("👨🏻‍🏫 ⚠️ Failed to run from current folder at\(FileSystem.shared.currentFolder.path) ⚠️")
            signPost.verbose("👨🏻‍🏫 Will try to run from folder defined in Info.plist with key \(ProjectFolderWorker.Key.scrRoot.rawValue) ...")
    ...
    public func attempt(_ asyncSwiftFormatAttemptOutput: (@escaping (@escaping SwiftFormatWorker.SyncOutput) -> Void))
    {
        ...
        { [weak self] in
            ...
            self.signPost.verbose("👨🏻‍🏫 \(self.folderToFormat.name) attempt ...")

            CLI.print = { message, type in
                switch type {
                case .info, .success, .content:
                    self.signPost.message("👨🏻‍🏫 \(self.folderToFormat.name) \n\(message)\n")
                case .error:
